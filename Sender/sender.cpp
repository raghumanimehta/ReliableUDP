...
Sender::Sender(const std::string &destIp, const int port)
  ...
  LOG_INFO(std::string("[SENDER-INIT] Successfully initialized. State: IDLE | "
                       "Destination: ") +
...
{
  if (this->socketFd < 0) {
    LOG_ERROR("[SEND-FILE] Invalid socket file descriptor. State: " +
  ...
  if (this->state != SenderState::CONNECTED) {
    LOG_ERROR("[SEND-FILE] State machine error. Expected: CONNECTED, Actual: " +
              senderStateToString(this->state));
    return false;
  }

  if (fileData.empty()) {
    LOG_WARNING("[SEND-FILE] Empty file data provided. State: CONNECTED | File "
                "size: 0 bytes");
    return true;
  }
  ...
  while (remainingSize > 0) {
    ...
    std::copy(fileData.begin() + offset, fileData.begin() + offset + thisSize,
              packetData.begin());
    offset += thisSize;
    ...
    if (pkt == nullptr) {
      LOG_ERROR("[SEND-FILE] Packet creation failed. SeqNo: " +
                std::to_string(seqNo) + " | Payload size: " +
                std::to_string(thisSize) + " | Flag: " + std::to_string(flag));
      return false;
    }
    if (!sendPacket(this->socketFd, this->dst, *pkt)) {
      LOG_ERROR(
          "[SEND-FILE] Failed to send packet. SeqNo: " + std::to_string(seqNo) +
    ...
    if (!this->window.add(std::move(ws))) {
      if (!this->waitForWindowToOpen()) {
        LOG_ERROR("[SEND-FILE] Failed while waiting for window to open.");
        return false;
      }
    }
    LOG_INFO("[SEND-FILE] Packet sent successfully. SeqNo: " +
             std::to_string(seqNo) +
             " | Payload size: " + std::to_string(thisSize) +
...
bool Sender::waitForWindowToOpen() {
  if (waitForReadWithRetry(socketFd, TIMEOUT_MS, RETRIES) != SUCCESS) {
    LOG_ERROR(
        "[WAIT-SYNACK] Timeout or error waiting for SYN-ACK. State: SYN_SENT");
    return false;
  }
  ...
  if (pkt->flag == FLAG_ACK && this->window.remove(pkt->seqNo)) {
    LOG_INFO(
        "[WAIT-WINDOW] Received ACK for SeqNo: " + std::to_string(pkt->seqNo) +
...
bool Sender::handshake() {
  ...
  LOG_INFO("[HANDSHAKE] Sending SYN packet. State: IDLE -> SYN_SENT | SeqNo: " +
  ...
  LOG_INFO(
      "[HANDSHAKE] State transition: SYN_SENT | Waiting for SYN-ACK response");
...
bool Sender::waitForSynAck() {
  ...
  if (pkt->flag == FLAG_SYN_ACK && pkt->seqNo == (SYN_SEQNO + 1)) {
    LOG_INFO("[WAIT-SYNACK] Received valid SYN-ACK. State: SYN_SENT -> SYN_ACK "
             "| SeqNo: " +
             std::to_string(pkt->seqNo) + " | Flag: SYN_ACK");
    this->state = SenderState::SYN_ACK;
    return true;
  }
  LOG_WARNING("[WAIT-SYNACK] Received packet with unexpected values. SeqNo: " +
              std::to_string(pkt->seqNo) +
              " | Flag: " + std::to_string(pkt->flag) + " | Expected SeqNo: " +
              std::to_string(SYN_SEQNO + 1) + " | Expected Flag: SYN_ACK");
  return false;
}
